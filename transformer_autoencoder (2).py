...
Original file is located at
    https://colab.research.google.com/drive/1lealB8729oM-1FkGaFPjgJafU8kL_aOy
"""
...
a=train_data_scaled- predicted_train
...
def laplFunc():
  ...
  return K
svm = OneClassSVM(kernel='rbf', gamma=h, nu=h)
print(svm)

predicted_normal = vae_model.predict(normal_train_data)



error_vector=(normal_train_data- predicted_normal)/np.max(normal_train_data-predicted_normal)
nsamples, nx, ny = error_vector.shape
d2_error = error_vector.reshape((nsamples,nx*ny))

svm.fit(d2_error,None)
pred = svm.predict(d2_error)

scores = svm.score_samples(d2_error)
thresh = quantile(scores, 0.03)
print(thresh)
3.994389673293594

index = where(scores<=thresh)
values = d2_error[index]

plt.scatter(d2_error[:,0], d2_error[:,1])
