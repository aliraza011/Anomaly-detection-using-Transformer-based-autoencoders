...
def sample(args):
    ...
    return z_mean + K.exp(0.5 * z_log_var) * epsilo
...
vae_model=classifier

X_pred = vae_model.predict(normal_test_data)



reconstructions=classifier.predict(normal_test_data)
...
plt.hist(mae_vector_an , bins=50,label='abnormal')
plt.hist(mae_vector , bins=50, label='normal')
plt.axvline(threshold,color='r', linewidth=3, linestyle='dashed', label='Threshold')
...
x1=reconstructions_an[25]
x2=anomaly_test_data[25]
def Average(lst):
